...
class UsageStatsPlugin extends GenericPlugin {
	...
	function register($category, $path, $mainContextId = null) {
		...
		if ($this->getEnabled($mainContextId) && $success) {
			...
			if ($this->getSetting(PKPApplication::CONTEXT_ID_NONE, 'createLogFiles')) {
				Hook::add('UsageEventPlugin::getUsageEvent', array(&$this, 'logUsageEvent'));
			}
	...
	function displayReaderStatistics() {
		...
		switch($applicationName) {
			...
			Hook::add('Templates::Article::Main', array($this, 'displayReaderArticleGraph'));
				break;
			case 'omp':
				// Add chart to book view page
				Hook::add('Templates::Catalog::Book::Main', array($this, 'displayReaderMonographGraph'));
				break;
			case 'ops':
				// Add chart to preprint view page
				Hook::add('Templates::Preprint::Main', array($this, 'displayReaderPreprintGraph'));
				break;
			default:
				assert(false);
		}
